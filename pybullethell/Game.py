...
class Game:
    ...
    """ Класс предназначен для главного игрового процесса: размер дисплея display_width, display_height; начальная позиция фона background_position; скорость скролла фона	backgorund_speed"""
    ...
    def __init__(self):
        ...
        """ Этот метод является конструктором класса Game"""
    ...
    def GameLoop(self):
        """ Гланый метод игрового цикла, он определяет продолжается ли игра, либо нет и вызывает необходимые функции во время жизни игрока self.player_alive, а именно: доступность управления self.key_action(), движение пуль self.bullets_movement()  , 
        спавн врагов self.enemy_spawner.spawn(), self.enemy_spawner.move(), выстрелы self.enemy_spawner.Shoot, дальнейшую прорисовку игры self.draw_display(); при поражении останавливает игру self.game_over()"""
        game = True
    ...
    def player_alive(self):
        """ Этот метод проверяет жив ли игрок player.health > 0"""
        return self.player.health > 0

    def check_bullets(self):
        """ Этот метод проверяет коллизию пули с другими объектами collided_enemy is not None
            Коллизия с игроком либо врагом отнимает ему здоровье collided_enemy.health -= i.damage и пуля удаляется self.bullets.remove(i)
         """
    ...
    def restart(self):
        """ Метод нужен для перезапуска игры при нажатии игроком определенной клавиши keys[pygame.K_r], в таком случае игра возообновляется"""
    ...
    def key_action(self):
        """ Метод нужен для взаимодествия игроком с дввижением персонажа: при использовании хоткеев:keys[pygame.K_a], keys[pygame.K_d], keys[pygame.K_w], keys[pygame.K_s] - 
        модель игрока меняет местоположение на const  """
        keys = pygame.key.get_pressed()
    ...
    def bullets_movement(self):
        """ Этот метод двигает все пули self.bullets"""
        for i in self.bullets:
            i.move()

    def draw_display(self):
        """ Метод рисует графику: а именно прорисовывает фон  self.background, пули self.bullets, врагов  self.enemy_spawner.enemies, а еще очки и жизни персонажа: self.print_text"""
