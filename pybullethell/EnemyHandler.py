...
class EnemyType:
    ...
    def __init__(self, health, cooldown, speed, sprite, bullet_spite, heal = False):
        ...
        """ Конструктор для класса, принимает здороввье health, время перезарядки cooldown, скорость передвижения speed,
         графику sprite, графику пули  bullet_spite и является ли враг лечительным heal = False"""
...
class Enemy:
    """ Класс врага, враг имеет определенные габариты width = 90, height = 90, начальные координаты x, y и начальную перезарядку cooldown """
    width = 90
    height = 90
    x = 0
    y = 0  
    ...
    def __init__(self, x, y, other):
        """ Конструктор класса, принимает значения начальной позиции self.x = x, self.y = y , а так же тип врага """
    ...
    def shoot(self):   
        """ Метод создаёт пулю для выстрела и возращяет её как объект BulletHandler.Bullet"""     
...
class EnemySpawner:
    """ Класс для создания врагов enemies, а так же интервал их появления в диапазоне min_cooldown_time = 50 - max_cooldown_time = 150 """
    ...
    def __init__(self, max_x, max_y):
        """ Конструктор класса, принимает значения минимального и максимального времени между созданием врагов  enemy1, enemy2, enemy3"""
        self.max_x = max_x
        self.max_y = max_y
    ...
    def spawn(self):
        """ Метод создаёт врага в определённом временном интервале . Враг создается за экраном pos_y = -100 """
        self.cooldown -= 1
        if(self.cooldown <= 0):
    ...
    def move(self):
        """ Метод двигает врага self.enemies в определённом напралвении (на игрока) """
        for i in self.enemies:
            i.y += i.speed

    def Shoot(self):
        """ Метод нужен для выстрела пули bullets. Частота выстрелов определяется временем перезаряжания i.default_cooldown"""
        bullets = []
        for i in self.enemies:
            if i.cooldown <= 0:
